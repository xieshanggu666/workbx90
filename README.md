...
3. **成员角色（权限隔离）**：
   - **成员**：播种/浇水/施肥/除虫/收获、买卖交易、动物领养与养护、加工排产/队列重排/退料（仅自己的工单）/入库、灌溉启停与地块水分设置、育种养护、**提交灾损申报与补证**
   - **管理员**：以上全部 + 推进时间与**灾害结算**、建造/拆除灌溉防灾设施、发起杂交试验、邀请管理、**灾损复核（通过/驳回/要求补证）**
...
8. **灾损申报与协作复核**：灾害天气造成损失后，成员可在「🌪️ 灾损」页按事件申报作物/动物/设施损失
   （描述 + 数量 + 申请金币/物资补偿）；管理员/场主协作复核——**不能审核自己的申报**，
   通过后在同一事务内联动发放金币与防灾物资并全农场广播；可**驳回**（驳回后释放名额可重新申报）、
   可**要求补证**（申报人补充材料后回到待复核队列）；
   **重复申报约束**：同一成员 × 同一灾害事件 × 同一类别仅允许一条进行中的申报（数据库部分唯一索引兜底）
9. **灌溉**：花金币建造蓄水池、沿空地铺设水渠连接地块；每块耕地可单独设置**目标水分**（浇到该水位为止，设为 0 则不自动浇水）与**保水优先级**（低/中/高）。
...
10. **杂交育种**：在「🧬 育种」页投入两批作物（同类或跨类，各 2 个 + 金币）开始试验，
...
> `disaster_claims` 记录灾损申报（事件/类别/申请与实赔金额/补证/复核留痕），
> 以部分唯一索引 `idx_claims_active_dedup` 约束「同成员×同事件×同类别的活跃申报唯一」；
...
> `server/production.test.mjs`（用临时库自清理）；
> 灾损申报（重复申报约束/协作复核禁自审/通过联动金币物资/驳回重报/补证流转）的集成测试在
> `server/claims.test.mjs`；或直接 `npm test` 跑全部。
