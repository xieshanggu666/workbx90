<template>
  <div class="tabs">
    <button v-for="t in tabs" :key="t.key" :class="{active:tab===t.key}" @click="tab=t.key">
      ...
      <span v-if="t.key==='claims' && claimsTodo" class="badge claim-badge">{{ claimsTodo }}</span>
    </button>
  </div>
...
// 待办角标：管理员看待审核/待补证数量；成员看自己需要补证的数量
const claimsTodo = computed(() => {
  if (store.canManage) return store.claims.filter((c) => c.status === 'pending').length
  return store.claims.filter((c) => c.status === 'moreinfo' && c.created_by === store.user?.id).length
})
